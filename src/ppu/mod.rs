...
impl Default for PPU {
    fn default() -> Self {
        Self::new(Arc::new([0u8; 0x800]), Mirroring::Horizontal)
    }
}

impl PPU {
    pub fn new(chr_rom: Arc<[u8]>, mirroring: Mirroring) -> Self {
        Self {
            chr_rom,
            mirroring,
...
pub mod test {
    ...
    fn test_vram_vertical_mirror() {
        let mut ppu = PPU::new(Arc::new([0; 2048]), Mirroring::Vertical);
    ...
    fn test_read_status_resets_vblank() {
        let mut ppu = PPU::default();
        ppu.status.set(StatusRegister::VBLANK, true);
