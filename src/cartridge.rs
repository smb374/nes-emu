...
impl Rom {
    pub fn new(raw: &[u8]) -> Result<Self, &'static str> {
        ...
        Ok(Rom {
            prg_rom: Arc::from(&raw[prg_rom_start..(prg_rom_start + prg_rom_size)]),
            chr_rom: Arc::from(&raw[chr_rom_start..(chr_rom_start + chr_rom_size)]),
            mapper: mapper,
...
pub mod test {
    ...
    fn test() {
        ...
        assert_eq!(rom.chr_rom.to_vec(), vec!(2; 1 * CHR_ROM_PAGE_SIZE));
        assert_eq!(rom.prg_rom.to_vec(), vec!(1; 2 * PRG_ROM_PAGE_SIZE));
